...
public class TS_FilePdfBoxUtils {
    ...
    public static TGS_UnionExcuse<Path> toJpg(Path pdfSrcFile, Path jpgDstFile, int pageNumber) {
        var u_deleteFileIfExists = TS_FileUtils.deleteFileIfExists(jpgDstFile);
        if (u_deleteFileIfExists.isExcuse()) {
            return u_deleteFileIfExists.toExcuse();
        }
        if (TS_FileUtils.isExistFile(jpgDstFile)) {
            return TGS_UnionExcuse.ofExcuse(d.className, "toJpg", "I could not able to delete image");
        }
        try (var doc = Loader.loadPDF(new RandomAccessReadBufferedFile(pdfSrcFile.toAbsolutePath().toString()))) {
            ...
            if (!result) {
                return TGS_UnionExcuse.ofExcuse(d.className, "toJpg", "ImageIO.write returned false");
            }
        } catch (IOException ex) {
            return TGS_UnionExcuse.ofExcuse(ex);
        }
        if (!TS_FileUtils.isExistFile(jpgDstFile)) {
            return TGS_UnionExcuse.ofExcuse(d.className, "toJpg", "I could not able to write image");
        }
        return TGS_UnionExcuse.of(jpgDstFile);
    }

    @Deprecated //TODO: I just wrote it. Not Tested!
    public static TGS_UnionExcuseVoid combine(List<Path> pdfSrcFiles, Path pdfDstFile) {
        try {
            var u_deleteFileIfExists = TS_FileUtils.deleteFileIfExists(pdfDstFile);
            if (u_deleteFileIfExists.isExcuse()) {
                return u_deleteFileIfExists;
            }
            if (TS_FileUtils.isExistFile(pdfDstFile)) {
                return TGS_UnionExcuseVoid.ofExcuse(d.className, "combine", "I could not able to delete the file");
            }
            ...
            if (!TS_FileUtils.isExistFile(pdfDstFile)) {
                return TGS_UnionExcuseVoid.ofExcuse(d.className, "combine", "I could not able to write the file");
            }
    ...
    public static TGS_UnionExcuseVoid extract(Path pdfSrcFile, int pageNr, Path pdfDstFile) {
        ...
        if (TS_FileUtils.isExistFile(pdfDstFile)) {
            return TGS_UnionExcuseVoid.ofExcuse(d.className, "extract", "I could not able to delete the file");
        }
        try (var doc = Loader.loadPDF(new RandomAccessReadBufferedFile(pdfSrcFile.toAbsolutePath().toString()))) {
//                var fromPage = pageNr;
//                var toPage = pageNr;
...
//                pdfDocPartial.save(pdfDstFile.toFile());
        ...
        if (!TS_FileUtils.isExistFile(pdfDstFile)) {
            return TGS_UnionExcuseVoid.ofExcuse(d.className, "extract", "I could not able to write the file");
        }
        return TGS_UnionExcuseVoid.ofVoid();
    }
...
//                        var pdfDocPartial = lst.get(0);
    ...
    public static TGS_UnionExcuseVoid castFromPDFtoHTM(Path srcPDF, Path dstHTM, CharSequence optionalTitle, CharSequence optionalHeaderContent, CharSequence optional_iframe_video, boolean addLoader) {
...
//        });
        ...
        var u_strHtm = TS_FileTxtUtils.toString(dstHTM);
        if (u_strHtm.isExcuse()) {
            return u_strHtm.toExcuseVoid();
        }
        var strHtm = u_strHtm.value();
        if (addLoader) {
...
//        });
//    }
    public static TGS_UnionExcuse<PDImageXObject> getImage(Path imgFile, PDDocument document) {
        try {
            return TGS_UnionExcuse.of(PDImageXObject.createFromFile(imgFile.toAbsolutePath().toString(), document));
        } catch (IOException ex) {
            return TGS_UnionExcuse.ofExcuse(ex);
        }
    }

    public static TGS_UnionExcuse<PDImageXObject> getImage(BufferedImage bi, PDDocument document) {
        try {
            return TGS_UnionExcuse.of(LosslessFactory.createFromImage(document, bi));
        } catch (IOException ex) {
            return TGS_UnionExcuse.ofExcuse(ex);
        }
    }

    public static TGS_UnionExcuseVoid insertImage(PDDocument document, PDPage page, PDImageXObject pdImage, int offsetX, int offsetY, float scale) {
    ...
    public static TGS_UnionExcuse<List<Path>> castFromIMGtoPDF_A4PORT_AllFiles(Path directory, boolean skipIfExists, boolean deleteIMGAfterConversion) {
        var u_subFiles = TS_DirectoryUtils.subFiles(directory, null, false, false);
        if (u_subFiles.isExcuse()) {
            return u_subFiles.toExcuse();
        }
        ...
        return TGS_UnionExcuse.of(convertedFiles);
    }

    public static TGS_UnionExcuseVoid castFromIMGtoPDF_A4PORT(Path srcIMG, Path dstPDF) {
        d.cr("castFromJPGtoPDF", "init", srcIMG, dstPDF);
        var u_deleteFileIfExists = TS_FileUtils.deleteFileIfExists(dstPDF);
        if (u_deleteFileIfExists.isExcuse()) {
            return u_deleteFileIfExists;
        }
        var u_img = TS_FileImageUtils.readImageFromFile(srcIMG, true);
        if (u_img.isExcuse()) {
            return u_img.toExcuseVoid();
        }
        var u_bi = TS_FileImageUtils.autoSizeRespectfully(u_img.value(),
                new TGS_ShapeDimension(612, 792),
                0.8f
        );
        if (u_bi.isExcuse()) {
            return u_bi.toExcuseVoid();
        }
        var bi = (BufferedImage) u_bi.value();
        try (var document = new PDDocument();) {
            var blankPage = new PDPage();
            document.addPage(blankPage);
            var u_pdImage = getImage(bi, document);
            if (u_pdImage.isExcuse()) {
                return u_pdImage.toExcuseVoid();
            }
            insertImage(document, blankPage, u_pdImage.value(), 0, 0, 1f);
    ...
    public static TGS_UnionExcuseVoid createPageBlank(Path path) {
        try (var document = new PDDocument();) {
            var blankPage = new PDPage();
            document.addPage(blankPage);
