...
public class Text {
    ...
    public static void ShowTextWithPositioning(String message, String outfile, float fontSize_20) throws IOException {
        // the document
        try ( PDDocument doc = new PDDocument();  InputStream is = PDDocument.class.getResourceAsStream("/org/apache/pdfbox/resources/ttf/LiberationSans-Regular.ttf")) {
            ...
            float stringWidth = font.getStringWidth(message) * fontSize_20;

            // Get the string height in text space units.
            float stringHeight = font.getFontDescriptor().getFontBoundingBox().getHeight() * fontSize_20;
            ...
            try ( PDPageContentStream contentStream = new PDPageContentStream(doc,
                ...
                List text = TGS_ListUtils.of();
                String[] parts = message.split("\\s");

                float spaceWidth = (justifyWidth / (parts.length - 1)) / fontSize_20;
                ...
                float extraLetterWidth = (justifyWidth / (message.codePointCount(0, message.length()) - 1)) / fontSize_20;
    ...
    public static void HelloWorld(String message, String filename) throws IOException {
